...
contract KycProviderImpl is Secured, KycProvider {
    ...
    event DataChange(address indexed addr, uint256 jurisdiction, bytes32 data);

    function resolve(address _address) public returns (Investor) {
        return data[_address];
    }

    function setData(address _address, uint256 _jurisdiction, bytes32 _data) ownerOr("operator") public {
        setDataInternal(_address, _jurisdiction, _data);
    }

    function setDataInternal(address _address, uint256 _jurisdiction, bytes32 _data) internal {
