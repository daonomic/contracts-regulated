...
contract KycProviderImpl is Secured, KycProvider {
    ...
    event DataChange(address indexed addr, uint128 jurisdiction, bytes16 data);

    function resolve(address _address) public returns (Investor) {
        return data[_address];
    }

    function setData(address _address, uint128 _jurisdiction, bytes16 _data) ownerOr("operator") public {
        setDataInternal(_address, _jurisdiction, _data);
    }

    function setDataInternal(address _address, uint128 _jurisdiction, bytes16 _data) internal {
